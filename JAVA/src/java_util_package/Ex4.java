...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * 	 LocalDateTime 클래스 : 날짜, 시각 관련 기능 제공
		 * - toString() 메서드 오버라이딩 되어 있음
		 *   각 클래스의 now() 메서드 호출하여 현재 시스템의 정볼르 가져오고, 
		 *   of() 메서드를 호출하여 날짜 및 시각 정보를 설정 가능
		...
		System.out.println(date); // 2022-10-24
		
		LocalTime time = LocalTime.now(); // 시스템의 시각 정보 리턴받음
		System.out.println(time); //11:30:41.425225

		LocalDateTime dateTime = LocalDateTime.now(); // 날짜, 시각 정보 리턴받음
		System.out.println(dateTime); //2022-10-24T11:31:21.115073600
...
//		Month month = date.getMonth(); // => 주의! getMonth() 메서드는 Month타입 객체 리턴됨
		int day = date.getDayOfMonth();
		
		System.out.println(year+"/"+month+"/"+day); // 2022/10/24
		
		
		// Month 타입 객체 활용
		// => enum 타입 객체인 Month 타입으로 관리됨 (각 월의 이름이 상수로 제공)
		Month enumMonth = date.getMonth();
		System.out.println(enumMonth); //OCTOBER
		String monthName = enumMonth.toString(); // 변수 저장시 String 변환 필수!
		...
		System.out.println(hour+":"+min+":"+sec); // 11:43:58
		...
		System.out.println(today.plusMonths(8)); // 8개월 뒤 날짜 설정 //2023-06-24
		System.out.println(today.plusDays(30)); // 30일 뒤 날짜 설정 //2022-11-23
		
		System.out.println(today.plusMonths(1).plusDays(15)); //2022-12-09
		...
		// 날짜와 시각 정보 비교를 위해
		// compardTo(), isAfter(), isBefore(), isEqual() 등의 메서드 제공됨
		
		// 1. comparTo()
		// => 현재 객체 날짜가 비교대상 객체보다 이전이면 음수, 이후면 양수, 같으면 0리턴함
		System.out.println(date3.compareTo(date4)); //-22
		
		// 2. isAfter()
		System.out.println("현재 날짜가 더 미래인가?" + date3.isAfter(date4)); //현재 날짜가 더 미래인가?false
		
		// 3. isBefore()
		System.out.println("현재 날짜가 더 과거인가?" + date3.isBefore(date4)); //현재 날짜가 더 과거인가?true
		
		// 4. isEqual()
		System.out.println("설정된(기준) 날짜가 현재 날짜와 동일한가? " + date3.isEqual(date4)); //설정된(기준) 날짜가 현재 날짜와 동일한가? false
