...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * => 즉, 서브클래스의 인스턴스를 통해 슈퍼클래스의 멤버에 접근이 불가능하게 된다!
		 * => (접근을 해야할 경우 사용 )
		 * 
		 *  < 레퍼런스 super >
		 ...
		 */
			...
			c.watchTv();
												//우리집 TV: 내가 구입한 TV
												//서브클래스에서 오버라이딩 된 watchTv() 메서드
												//독립한 나의 집에서 내가 구입한 TV 보기!
			...
			class Child extends Parent {
				...
				public void watchParentTv() {
					//현재 인스턴스 내에서 오버라이딩으로 인해 은닉된 슈퍼클래스 멤버는
			 ...
			 * - 최종적으로 Object클래스까지 거슬러 올라가면서 탐색하여 없으면 오류 발생!
			 * 
			 */
			...
			class ChildClass extends ParentClass {
				...
				public void method() {
					String x = "method"; // 서브클래스의 메서드 내의 로컬변수 선언
					
					System.out.println("x = " + x); // 로컬변수 메서드 출력 됨
					// => 만약, 로컬변수를 선언하지 않을 경우 this.x와 동일
					
					System.out.println("this.x = " + this.x); // 서브클래스 멤버변수x
					// => 만약, Child 클래스 멤버변수를 선언하지 않을 경우 super.x와 동일
					
					System.out.println("super.x = " + super.x); // 슈퍼클래스 멤버변수x
