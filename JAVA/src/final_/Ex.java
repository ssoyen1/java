...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 	  => 클래스 변경 불가 = 상속 금지!
		 * 
		 * Getter Setter 만들때 fianl 붙은 것은 set 안나옴. 값이 안변하니 쓸 필요도 없음.
		 * 
		 */
...
//				Math.PI = 3.14; // 컴파일러 에러 발생! 값 변경 불가
				...
				class FinalMember {
					...
					// => 초기화 하지 않으면 아래의 오류 발생
//					final int finalMember;
										// => The blank final field finalMember may not have been initialized
										//	  (초기값을 주지않아서 발생한 에러 <= 정수값주면 에러사라짐)
					final int finalMember = 10;
					
					// 만약, final 상수를 초기화하지 않을 경우
					final int blackFinalMember; // 컴파일에러 발생!
												// => 단, 파라미터 생성자를 통해 초기화하는 경우 사용이 가능!
												//		  (alt shift s + v)
					
					// blank final 상수를 초기화하는 생성자 정의 필수!
					public FinalMember(int blackFinalMember) {
						super();
						this.blackFinalMember = blackFinalMember; // blank final 상수 초기화
					}
					
					
					public void change() {
//						finalMember = 99; // 오류발생! The final field FinalMember.finalMember cannot be assigned
										  // => final 키워드가 붙은 변수(=상수)는 값을 변경할 수 없다.
										  // 	이름은 사용 가능
						System.out.println("finalMember = " + finalMember);
						
						final int finalLocalVar = 20; // 로컬변수에도 final키워드 사용 가능
...
//					}
					...
					public void normalMethod() {
						...
						finalMethod(); // 오버라이딩 할 수는 없지만 호출은 가능
					}
...
//														  // => final 클래스를 상속받을 수 없다!
				...
				class SubFinalClass {
					...
					FinalClass fc = new FinalClass(); // 인스턴스 생성-> 참조변수 통해 멤버변수에 접근 가능
...
//				class SubString extends String {} // String 클래스 상속 불가!
												  // String 에러뜬곳에 f1누르면 Javadoc for 'java.lang.String' 뜸
												  // : java.lang.Object를 상속받는 java.lang.String
//				class Submate extends Math {}     // Math 클래스 상속 불가!
