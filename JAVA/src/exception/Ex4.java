...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * - 사용자가 직접 예외 클래스를 작성하여 
		 *   발생시킬 예외에 맞는 객체로 만들어서 전달하면 예외 파악이 쉬워진다
	...
	// 사용자 정의 예외 클래스 작성 (예외를 발생하면 객체가 생김. 클래스가 만들어진다는뜻)
	// - 원하는 이름의 서브클래스 정의 후 예외 클래스 상속받음
	// - 주로, Exception 클래스를 상속받아 정의함
	class InvalidScoreException extends Exception {
		
		// 생성자를 정의하여 예외 메세지를 슈퍼클래스의 생성자에 전달
		public InvalidScoreException(String message) { // 메세지를 매개변수에 넣으면 이셉션(부모변수)에 전달한다.
			super(message); // 슈퍼라는 생성자가 숨겨져있음 .얘에 의해서 이셉션(부모)의 객체가 먼저만들어짐
		}
