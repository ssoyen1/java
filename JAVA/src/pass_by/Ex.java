...
public class Ex {
		public static void main(String[] args) {
			 ...
			 * 	 => 실제 값이 있는 공간의 주소 값(참조값)을 복사해서 전달
			...
			// money로부터 인스턴스를 만들어야 참조변수에 접근이 가능하게됨.  // 참조변수(멤버변수 또는 멤버 메서드를 사용하기위해사용)
			Money m = new Money();
			PassByTest pbt = new PassByTest(); //기본생성자통해 인스턴스2개만들기
			
			
			
			
			
			//1. 메서드 파라미터로 기본 데이터타입 변수값을 전달하는 경우
			...
			pbt.changePrimitiveType(m.money); // money에 저장된 실제값(10000)
			...
			class PassByTest {
				public void changePrimitiveType(int money) { // 실제 데이터타입을 적었 -> money는 여전히 10000
					// 기본 데이터타입 데이터(실제값)을 전달받음(복사)
				...
				public void changeReferenceType(Money m) { // 주소값을 받음 -> money값이 변경될 수 있다
														   // 타입이 기본형 아님 -> 참조변수
			...
			class Money {
				int money = 10000;
					...
					// 반환타입이 참조형일 경우 객체의 주소(객체.just 정수.int값)를 반환.
					// 반환받는 객체의 타입도 참조형이여야함.
					// -> 메인메서드 안에서 참조변수를 사용할 수 있음.
