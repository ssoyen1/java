...
public class Ex3 {
	...
	public static void main(String[] args) {
		 ...
		 *    	해당 인스턴스를 다루는 관계
		 *    - 예) 자동차가 - 엔진기능을 포함한다, 스마트폰이 - 카메라기능을 포함한다.
		 *    
		 *    
		 * 2. is-a 관계(상속)
		 ...
		 */
			...
			// Q. 자동차(car)와 엔진(Engine)의 관계는? 
			// A. has-a(포함) 관계이다.
			// => 자동차는 엔진을 포함한다 = Car has a Engine.
			class Car {
				// 포함관계일 경우 해당 클래스의 인스턴스를 통해 접근 가능한 관계이다.
				Engine engine = new Engine(); // 인스턴스 만들기
			...
			class FireEngine extends Car {
				...
				public void showFireEngineInfo() {
					...
					// 부모의 멤버에 접근
					System.out.println("모델명 : " + modelName); //상속관계에 있으면 바로 멤버변수 사용가능
					
					// 부모와 포함관계인 Engine 객체의 멤버에 접근하는 법
					// 인스턴스 만든 적은 없지만 
					System.out.println("배기량 : " + engine.cc); // 부모에 있는 engine도 바로 가져다 사용할 수 있음
				}
			}
